...
def psuccessors(state, N=N):
    ...
    carros   = [c for c in state if c[0] not in '@|']
    ...
    def psucc_one_dir(start, i):
        ...
        while board[start+(i*(1 if i<0 else n))] in '.@':
            sqs_moved += i
            nuevo_carro = (car, locs(start+i, n, abs(i)))
            carros = [nuevo_carro if c==(car,sqs) else c for c in carros]
            new_state = tuple(carros + goal + border)
            sucesores[new_state] = (car, sqs_moved)
            start += i

    for car, sqs in carros:
        cstart = sqs[0]
        n = len(sqs)
...
def shortest_path_search(start, successors, is_goal):
    ...
    if is_goal(start):
        return [start]
    explorado = set() # set of states we have visited
    frontera = [ [start] ] # ordered list of paths we have blazed
    while frontera:
        path = frontera.pop(0)
        s = path[-1]
        for (estado, action) in successors(s).items():
            if estado not in explorado:
                explorado.add(estado)
                path2 = path + [action, estado]
                if is_goal(estado):
                    return path2
                else:
                    frontera.append(path2)
    return []
