...
def find_best_flights(vuelos, origen, destino):
	grafo_aereo = construir_grafo_aereo(todos_los_vuelos)

	posible_trayectoria = []

	for ciudades in obtener_posible_trayectoria(grafo_aereo, origen, destino):
		posible_trayectoria += [trayectoria_a_ciudades(grafo_aereo, ciudades)]

	feasible = []
	for item in posible_trayectoria:
		feasible += filter(lambda x: valid_line(vuelos, x), item)

	if not feasible:
		return None

	vuelos_baratos = eficiencia_del_costo(vuelos, feasible)

	return list(eficiencia_del_tiempo(vuelos, vuelos_baratos))

def eficiencia_del_costo(flights, path_list):
...
def eficiencia_del_tiempo(vuelos, lista_trayectorias):
	return min(lista_trayectorias, key=lambda x: obtener_duracion_trayectoria(vuelos, x))

def obtener_duracion_trayectoria(vuelos, trayectoria):
	duraciones_temporales = map(lambda x: obtener_duracion_aerolinea(vuelos, x), trayectoria)

	return reduce(operator.add, duraciones_temporales)

def obtener_duracion_aerolinea(vuelos, aerolinea):
	duracion = obtener_tiempo_en_aire(vuelos, aerolinea)

	return duracion[1] - duracion[0]

def get_path_cost(vuelos, trayectoria):
	costo_aerolinea = lambda x: id_to_airline(vuelos, x)[5]

	return sum(map(costo_aerolinea, trayectoria))

def construir_grafo_aereo(vuelos):
	G = defaultdict(lambda : defaultdict(list))

	for aerolinea in vuelos:
		origen = aerolinea[1]
		destino   = aerolinea[2]
		G[origen][destino] += [aerolinea[0]]

	return G

def obtener_posible_trayectoria(grafo_aereo, origen, destino):
	accessible = []
	frontera  = [[origen]]
	while frontera:
		trayectoria_actual = frontera.pop()
		ultima_parada = trayectoria_actual[-1]

		if ultima_parada == destino:
			accessible += [trayectoria_actual]

		for siguiente_parada in grafo_aereo[ultima_parada]:
			if siguiente_parada not in trayectoria_actual:
				frontera += [trayectoria_actual+[siguiente_parada]]

	return accessible

def trayectoria_a_ciudades(grafo_aereo, trayectoria_de_ciudad):
	lista_aerolineas = []
	for i in range(len(trayectoria_de_ciudad) - 1):
		origen      = trayectoria_de_ciudad[i]
		destino = trayectoria_de_ciudad[i + 1]
		lista_aerolineas += [grafo_aereo[origen][destino]]

	return list(itertools.product(*lista_aerolineas))

def id_to_airline(flights, ID):
	return filter(lambda x: x[0] == ID, flights)[0]

def obtener_tiempo_en_aire(flights, ID):
	partida, llegada = id_to_airline(flights, ID)[3:5]
	transferencia = lambda x: datetime.strptime(x, '%H:%M')

	return map(transferencia, (partida, llegada))

def valid_line(vuelos, trayectoria_aerolinea):
	ultima_llegada = datetime.strptime('00:00', '%H:%M')

	for airlineID in trayectoria_aerolinea:
		partida, llegada = obtener_tiempo_en_aire(vuelos, airlineID)

		if partida < ultima_llegada:
			return False

		ultima_llegada = llegada
...
#
todos_los_vuelos = [(523, 'Broome', 'Derby', '07:17', '08:57', 60),
...
def test():
    vuelos = find_best_flights(todos_los_vuelos, 'Mt Magnet', 'Fitzroy Crossing')
    assert vuelos == [314, 803, 348, 530, 112]

    vuelos = find_best_flights(todos_los_vuelos, 'Leonora', 'Fitzroy Crossing')
    assert vuelos == None

    vuelos = find_best_flights(todos_los_vuelos, 'Meekatharra', 'Wiluna')
    assert vuelos == [391, 459]
